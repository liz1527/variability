...
#    semtb = Table.read('SE_outputs_yearstacks/K_extraction/cleaned_'+sem+'_output_H.fits')
    if sem == '12B':
        semtb = Table.read('SE_outputs_yearstacks/K_extraction/cleaned_'+sem+'_output_H.fits')
    else:
        semtb = Table.read('SE_outputs_yearstacks/K_extraction/extra_clean_'+sem+'_output_H.fits')
...
xrayfull = np.isin(semcom['NUMBER_06B'], xraymf['NUMBER_06B'])
xraycolfull = Column(xrayfull, 'X-ray')
semcom.add_column(xraycolfull)

#%% Save the tables
extra = '_extra_clean'
semcom.write('mag_flux_tables/H/mag_flux_table_H'+extra+'.fits')
bestmf.write('mag_flux_tables/H/mag_flux_table_best_H'+extra+'.fits')
starsmf.write('mag_flux_tables/H/stars_mag_flux_table_H'+extra+'.fits')
xraymf.write('mag_flux_tables/H/xray_mag_flux_table_best_H'+extra+'.fits')