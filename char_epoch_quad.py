...
for m, qtbdata in enumerate(quaddata):
    ...
    fluxn = vari_funcs.flux6_stacks(qtbdata)
    fluxchann = vari_funcs.flux6_stacks(qchandata) 
    sfluxn = vari_funcs.flux6_stacks(qsdata)
    ...
    fluxerr = vari_funcs.fluxerr6_stacks(qtbdata)
    fluxerrchan = vari_funcs.fluxerr6_stacks(qchandata)
    sfluxerr = vari_funcs.fluxerr6_stacks(qsdata)
...
#    plt.legend()
    ...
    for n, binedge in enumerate(bins[0:np.size(bins)-1]):
        ...
        # get errors
        fluxerr = vari_funcs.fluxerr3_stacks(bindata)
        fluxchanerr = vari_funcs.fluxerr3_stacks(binchan)
        sfluxerr = vari_funcs.fluxerr3_stacks(sbindata)
        print(len(flux))
...
t = Table(sigdict)
t.write('quad_epoch_sigma_table_extra_clean_no06_4arcsec.fits')
