...
filename = 'mag_flux_tables/J/month/month_flux_table_best_J_extra_quad_clean.fits'
chanfilename = 'mag_flux_tables/J/month/month_xray_flux_table_best_J_extra_quad_clean.fits'
sfilename = 'mag_flux_tables/J/month/month_stars_flux_table_J_extra_quad_clean.fits'
...
for m, qtbdata in enumerate(quaddata):
    ...
    fluxn = vari_funcs.j_mag_flux.month_flux_stacks(qtbdata, aper=ap)
    fluxchann = vari_funcs.j_mag_flux.month_flux_stacks(qchandata, aper=ap) 
    sfluxn = vari_funcs.j_mag_flux.month_flux_stacks(qsdata, aper=ap)
...
#    sfluxn, qsdata = vari_funcs.flux_funcs.noneg(sfluxn, qsdata)
    ...
    fluxerr = vari_funcs.j_mag_flux.month_fluxerr_stacks(qtbdata, aper=ap)
    fluxerrchan = vari_funcs.j_mag_flux.month_fluxerr_stacks(qchandata, aper=ap)
    sfluxerr = vari_funcs.j_mag_flux.month_fluxerr_stacks(qsdata, aper=ap)
...
#                                     stars=True, normalised=True, scale='symlog')
    ...
    bins, medvar = vari_funcs.selection_plot_funcs.plot_median_line_stars_J(fluxn, 
                                                                          qtbdata, 
                                                                          sfluxn, 
                                                                          qsdata, 
                                                                          statistic='var',
...
#    plt.legend()
    ...
    for n, binedge in enumerate(bins[0:np.size(bins)-1]):
        ...
        # get errors
        fluxerr = vari_funcs.j_mag_flux.month_fluxerr_stacks(bindata, aper=ap)
        fluxchanerr = vari_funcs.j_mag_flux.month_fluxerr_stacks(binchan, aper=ap)
        sfluxerr = vari_funcs.j_mag_flux.month_fluxerr_stacks(sbindata, aper=ap)
...
#            plt.hlines(22.458,4e-1,1e7, label='99.9% confidence level', zorder=4)
            plt.hlines(78.11,4e-1,1e7, label='Chi=30', zorder=4)
            plt.legend()
        else:
...
#t = Table(sigdict) 
#t.write('sigma_tables/month_quad_epoch_sigma_table_J_extra_quad_clean_2arcsec_neg.fits', overwrite=True)
