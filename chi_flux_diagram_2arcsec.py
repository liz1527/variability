...
Code to create chi-flux plot using the bootstraped error bars in 2 acrsec K band
...
#varys = fits.open('variable_tables/no06_variables_chi30_2arcsec_DR11data_restframe_07B.fits')[1].data
...
#
varys = fits.open('variable_tables/no06_variables_chi30_2arcsec_SpUDSmatch.fits')[1].data
flux = vari_funcs.k_mag_flux.flux4_stacks(varys)
flux, varys = vari_funcs.flux_funcs.noneg(flux, varys)
flux, fluxerr, newvarys = vari_funcs.k_mag_flux.create_quad_error_array(sigtb, varys, aper=4)
meanflux = np.nanmean(flux, axis=1)
...
plt.plot(meanflux, chisq, 'ks', mfc='None', markersize=10, label='SpUDS Source')
plt.legend()
