...
mean_lag_high_ccf = np.zeros(num_bins)
...
#    max_lag_fit[n] = plot_tau[np.argmax(ccf_fit)]
    ...
    #%% FInd centroid from values above 80% max ccf ###
    high_ccf = ccf[ccf>0.8*np.max(ccf)]
    high_tau = tau_arr[ccf>0.8*np.max(ccf)]
    mean_lag_high_ccf[n] = ws.numpy_weighted_mean(high_tau, weights=high_ccf)
