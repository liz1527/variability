...
def plot_sig(tbdata, sigtb, binedge, aper=5, format='o',colour='b', label='Ensemble'):
    ...
    fluxnorm, fluxerrnorm, sig, sigerr, eddrat, meaneddrat = get_ensemble_sig(tbdata, sigtb, binedge,posvar, aper)
    binupper = np.append(binedge[1:],np.nanmax(eddrat))
    xlow = meaneddrat-binedge
    xhigh = binupper - meaneddrat
    plt.errorbar(meaneddrat, sig, xerr=[xlow, xhigh], yerr=sigerr, fmt=format,
                 color=colour, label=label)

#%% Find edd rat for chandra sources and create bins ###
### Find luminosity distance ###
z = chandata['z_spec']#[mask]
z[z==-1] = chandata['z_p'][z==-1]
DL = cosmo.luminosity_distance(z)
DL = DL.to(u.cm)
...
Ledd = 1.26e31 * (0.1*Mstar)
eddrat = np.log(10*xrayL) - np.log(Ledd)
sortededdrat = np.sort(eddrat)
posvar = np.linspace(0,2,5000)

binsize = 10
binedge = np.empty(int(len(eddrat)/binsize))
size = len(binedge)
plt.figure(figsize=[8,7])
for n in range(size):
    ### Define bin edges ###
    enXrayL = sortededdrat[n*binsize:(n*binsize)+binsize]
