...
tbdata = Table.read('mag_flux_tables/K/month/month_mag_flux_table_best_K_extra_quad_clean_edged.fits')#[1].data
sigtb = Table.read('sigma_tables/month_quad_epoch_sigma_table_K_extra_quad_clean_2arcsec_neg_allmonths.fits')

### remove low flux ###
flux = vari_funcs.k_mag_flux.month_flux_stacks(tbdata)
mask1 = np.nanmean(flux, axis=1) < 1e4
tbdata = tbdata[mask1]
...
### isolate high chisq ###
mask2 = vari_funcs.vary_stats.my_chisquare_err(flux, fluxerr) > 84

tbdata = Table(tbdata[mask2])
tbdata.write('mag_flux_tables/K/month/month_mag_flux_table_best_K_extra_quad_clean_lowflux_variables.fits')
del tbdata
...
meanerr = np.sqrt(np.sum(np.square(fluxerrn), axis=0))/len(fluxerrn)
