...
### Get data ###
nontb = fits.open('variable_tables/K/no06_variables_chi30_2arcsec_nochanXray_DR11data_restframe.fits')[1].data
xraytb = fits.open('variable_tables/K/no06_variables_chi30_2arcsec_chandata_DR11data_restframe.fits')[1].data
#nontb = Table.read('variable_tables/K/no06_variables_chi30_2arcsec_noXray_DR11data_restframe.fits')
#xraytb = Table.read('variable_tables/K/no06_variables_chi30_2arcsec_Xray_DR11data_restframe.fits')
sc = Table.read('variable_tables/K/no06_variables_chi30_2arcsec_DR11data_restframe_SC.fits')
...
sdata = fits.open('variable_tables/K/variables_stars_DR11data.fits')[1].data

### Limit to Chandra region for simplicity ###
nontb = vari_funcs.field_funcs.chandra_only(nontb)
xraytb = vari_funcs.field_funcs.chandra_only(xraytb)
fullxray = vari_funcs.field_funcs.chandra_only(fullxray)
sdata = vari_funcs.field_funcs.chandra_only(sdata)
#chandata = vari_funcs.field_funcs.chandra_only(chandata)
#fullUDS = vari_funcs.field_funcs.chandra_only(fullUDS)
...
su_v, sv_j = get_data(sdata, meanu, meanv, meanj)
...
plt.plot(fullv_j, fullu_v, '.',markersize=0.5, color='tab:grey', alpha=0.2, label='Galaxy', zorder=0, rasterized=True)
plt.plot(allxv_j, allxu_v, 's', markersize=5, color='k', label='Non-Variable X-ray AGN', zorder=1, rasterized=True)
plt.plot(noxv_j, noxu_v,'bo', label='Variable Non X-ray AGN', zorder=3)
plt.plot(xv_j, xu_v,'ro', label='Variable X-ray AGN', zorder=3)
#plt.plot(sv_j, su_v,'m*', label='Variable Star', zorder=4, markersize=10)
plt.legend()
