...
Kvarys = fits.open('variable_tables/K/variables_no06_chi30_negonly_DR11data.fits')[1].data
Jvarys = fits.open('variable_tables/K/variables_no06_chi30_negonly_DR11data_J.fits')[1].data
Hvarys = fits.open('variable_tables/K/variables_no06_chi30_negonly_DR11data_H.fits')[1].data
...
### Get K band data ###
Kflux = vari_funcs.k_mag_flux.flux4_stacks(Kvarys)
#Kflux, Kvarys = vari_funcs.noneg(Kflux, Kvarys)
Kflux, Kfluxerr, newKvarys = vari_funcs.k_mag_flux.create_quad_error_array(Ksigtb, Kvarys, aper=4)
...
### Get J band data ###
#Jflux = vari_funcs.j_mag_flux.flux4_stacks(Jvarys)
#Jflux, Jvarys = vari_funcs.flux_funcs.noneg(Jflux, Jvarys)
Jflux, Jfluxerr, newJvarys = vari_funcs.j_mag_flux.create_quad_error_array_J(Jsigtb, Jvarys, aper=4)
#Jfluxerr = vari_funcs.j_mag_flux.fluxerr4_stacks(Jvarys)
...
### Get H band data ###
Hflux = vari_funcs.h_mag_flux.flux4_stacks(Hvarys)
#Hflux, Hvarys = vari_funcs.flux_funcs.noneg(Hflux, Hvarys)
#Hflux, Hfluxerr, newHvarys = vari_funcs.h_mag_flux.create_quad_error_array_H(Hsigtb, Hvarys, aper=4)
Hfluxerr = vari_funcs.h_mag_flux.fluxerr4_stacks(Hvarys)
...
for n in range(len(newKvarys)): #Will plot all with blank plots of those with no J/H
    ...
    obnum = newKvarys['ID'][n]
    Jmask = np.isin(newJvarys['ID'], obnum)
    Hmask = np.isin(Hvarys['ID'], obnum)
...
#    
#    plt.tight_layout()
#    plt.savefig('plots/new_catalogue/Chi30Lightcurves/neg_only/JHK_lightcurves/flux_'+str(obnum))#+str(varys['NUMBER_05B'][n])+'_lightcurve.png')
#    plt.close('all')
